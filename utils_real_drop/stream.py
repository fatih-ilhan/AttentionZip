...
from utils_real_drop.modify_llama import AttZipLlamaForCausalLM_streaming
...
def parse_args():
    ...
    ## AttZip KV-Cache
    parser.add_argument("--imp_size", type=int, default=1)
    parser.add_argument("--enable_attzip_kv_cache", action="store_true")

    args = parser.parse_args()
    return args


def load(model_name_or_path, imp=False, args=None):
    ...
    dtype = torch.float16
    if imp:
        model = AttZipLlamaForCausalLM_streaming.from_pretrained(
        model_name_or_path,
        device_map="auto",
        config=config,
        torch_dtype=dtype,
